...
import { AliRouteCollectType } from '../types'

interface WxRouteTransformType {
  data: ReportDataType
  collectedData: AliRouteCollectType
}

const wxRoutePlugin: BasePluginType<WxEventTypes, AliClient> = {
  name: WxBaseEventTypes.ROUTE,
  monitor(notify) {
    monitorWxRoute.call(this, notify)
  },
  transform(collectedData: AliRouteCollectType): WxRouteTransformType {
...
export function monitorWxRoute(this: AliClient, notify: (eventName: WxBaseEventTypes, data: any) => void) {
      ...
      value: function (
        ...
        if (
          ...
          const failHandler:
            ...
            const failData: AliRouteCollectType = {
              ...data,
              isFail: true,
              message: res.errMsg
            }
