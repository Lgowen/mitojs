import { getBreadcrumbCategoryInWx, setUrlQuery, Severity, variableTypeDetection } from 'remitojs-utils'
import { DeviceInfo } from 'remitojs-types'
import { AliClient } from './aliClient'
import { WxBreadcrumbTypes } from 'remitojs-shared'
...
export async function getWxMiniDeviceInfo(): Promise<DeviceInfo> {
  const { pixelRatio, screenHeight, screenWidth } = my.getSystemInfoSync()
...
export function addBreadcrumbInWx(this: AliClient, data: any, type: WxBreadcrumbTypes, level = Severity.Info, params = {}) {
